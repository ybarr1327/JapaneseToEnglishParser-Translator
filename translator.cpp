...
//=================================================
// File translator.cpp written by Group Number: 09
//=================================================
...
//we used the unordered map for the dictionary part because it allows for fast lookup similar to something like a python dictionary
unordered_map<string,string> Dictionary; 

void setup_dictionary() 
{
  ifstream dictionaryRead ("lexicon.txt");//open the lexicon file
  
  if (dictionaryRead.is_open()) // if the file was opened
  {
    string key;
    string val;

    while(dictionaryRead >> key && dictionaryRead >> val) // read in the japanese and english words
    {
      Dictionary[key] = val; // store them in the dictionary
    }
  }
  else
  {
    cout<<"Unable to open lexicon.txt"; // if here then there was an error opening the file to exit
    exit(1);
  }
    

  dictionaryRead.close();//close the file when done with it
...
void getEword()
{
  if(Dictionary.find(saved_lexeme) == Dictionary.end()) // if the saved_lexeme is not found in the dictionary
  {
    saved_E_word = saved_lexeme; //store the japanese as the saved english word
  }
  else // if the saved_lexeme is found in the dictionary
  {
    saved_E_word = Dictionary[saved_lexeme]; //store the english word from the dictionary
  }
}
...
ofstream translatedContents; // this is the output file for the translation

void gen(string token)
{
  if(token == "TENSE") // if the token is tense
  {
    //output the token type
    translatedContents << "TENSE: " << tokentypeStrings(saved_token) << endl;
  } 
  else
  {
    //if the token is not tense then output the saved english word
    translatedContents << token << ": " << saved_E_word << endl;
  }
}
...
int main()
{
  ...
  //** closes lexicon.txt 
  //the input file is opened and closed in the setup_dictionary function
  setup_dictionary();
  

  //** opens the output file translated.txt
  translatedContents.open("translated.txt");
